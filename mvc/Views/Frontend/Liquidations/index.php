...
<head>
    <?php if( !empty($configInfo) ) : ?>
        <title>Sản phẩm thanh lý giá rẻ</title>
        ...
        <meta property="og:title" content="Sản phẩm thanh lý giá rẻ">
        <meta property="og:description" content="<?php {{ echo !empty($configInfo['config_metaDesc']) ? $configInfo['config_metaDesc'] : null; }} ?>">
        <meta property="og:url" content="<?php {{ echo Config::getBaseUrlClient("san-pham-thanh-ly-gia-re"); }} ?>">
        <meta property="og:site_name" content="Sản phẩm thanh lý giá rẻ">
        ...
        <meta name="twitter:title" content="Sản phẩm thanh lý giá rẻ">
                    ...
                    <ol class="breadcrum_list grid_row align_items_center">
                        ...
                        <li class="breadcrum_item nav3 d_flex align_items_center position_relative">
                            <a href="javascript:;" class="breadcrum_link" title="Sản phẩm thanh lý giá rẽ">
                                <span>Sản phẩm thanh lý giá rẽ</span>
                            </a>
                        </li>
                    </ol>
                </div>
                ...
                <div class="cate_view_content container grid_row">
                    <aside class="cate_content_sidebar">
                        ...
                        <?php if(!empty($listBrand)) : ?>
                            <?php {{ view("Frontend.Liquidations.brandFilter", [
                                    "listBrand"      => $listBrand,
                        ...
                        <?php endif; ?>
                        <?php {{ view("Frontend.Liquidations.priceFilter", [
                            "liquidationUrl" => $liquidationUrl,
                            "sortUrl"        => $sortUrl,
                            "priceArr"       => $priceArr,
                            "brandUrl"       => $brandUrl
                    ...
                    <div class="cate_content_main">
                        ...
                        <section class="home_cate_child">
                            <div class="cate_child_header">
                                <h3 class="cate_child_title">Sản phẩm thanh lý <span>(<?php {{ echo $numProdByLiquidation; }} ?> sản phẩm)</span></h3>
                            </div>
                        ...
                        <?php endif; ?>
                        <?php if( !empty($listProdByLiquidation) ) : ?>
                        <section class="product_box container_bg_sc">
                            <div class="option_box_wrap grid_row justify_content_between align_items_center">
                                <div class="sort_box_holder d_flex align_items_center">
                                    ...
                                    <ul class="list_sort d_flex">
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$liquidationUrl}{$brandUrl}{$priceUrl}/sort=lastest{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "lastest" ? "active" : ''; }} ?>">Hàng mới</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$liquidationUrl}{$brandUrl}{$priceUrl}/sort=bestsellers{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "bestsellers" ? "active" : ''; }} ?>">Bán chạy nhất</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$liquidationUrl}{$brandUrl}{$priceUrl}/sort=priceasc{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "priceasc" ? "active" : ''; }} ?>">Giá tăng dần</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$liquidationUrl}{$brandUrl}{$priceUrl}/sort=pricedesc{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "pricedesc" ? "active" : ''; }} ?>">Giá giảm dần</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="wrap-list-product-catalog grid_row">
                                <?php foreach($listProdByLiquidation as $prodItem) : ?>
                            ...
                            {{
                                if($totalPage > 1) { echo Pagination::getPagination("{$liquidationUrl}{$brandUrl}{$priceUrl}{$sortUrl}/page=", $totalPage, $page); }
                            }} ?></div>
                        </section>
                        <?php else: ?>
