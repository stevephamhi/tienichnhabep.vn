...
<head>
    <?php if( !empty($configInfo) ) : ?>
        <title><?php {{ echo !empty($strSearch) ? "Tìm kiếm " . $strSearch : "Trang tìm kiếm"; }} ?></title>
        ...
        <meta property="og:title" content="<?php {{ echo !empty($strSearch) ? "Tìm kiếm " . $strSearch : null; }} ?>">
        <meta property="og:description" content="<?php {{ echo !empty($configInfo['config_metaDesc']) ? $configInfo['config_metaDesc'] : null; }} ?>">
        <meta property="og:url" content="<?php {{ echo Config::getBaseUrlClient("search/?q={$strSearch}"); }} ?>">
        <meta property="og:site_name" content="<?php {{ echo Config::getBaseUrlClient("search/?q={$strSearch}"); }} ?>">
        ...
        <meta name="twitter:title" content="<?php {{ echo !empty($strSearch) ? "Tìm kiếm " . $strSearch : null; }} ?>">
                ...
                <div class="container">
                    <ol class="breadcrum_list grid_row align_items_center">
                        ...
                        <li class="breadcrum_item nav3 d_flex align_items_center position_relative">
                            <a href="javascript:;" class="breadcrum_link" title="Sản phẩm thanh lý giá rẽ">
                                <span>Tìm kiếm</span>
                            </a>
                        </li>
                    </ol>
                ...
                <div class="cate_view_content container grid_row">
                    <aside class="cate_content_sidebar">
                        ...
                        <?php if(!empty($listBrand)) : ?>
                            <?php {{ view("Frontend.Searchs.brandFilter", [
                                "listBrand" => $listBrand,
                                "searchUrl" => $searchUrl,
                                "brand_id"  => $brand_id,
                                "priceUrl"  => $priceUrl,
                        ...
                        <?php endif; ?>
                        <?php {{ view("Frontend.Searchs.priceFilter", [
                            "searchUrl" => $searchUrl,
                            "sortUrl"   => $sortUrl,
                            "priceArr"  => $priceArr,
                            "brandUrl"  => $brandUrl
                    ...
                    <div class="cate_content_main">
                        ...
                        <section class="home_cate_child">
                            <div class="cate_child_header">
                                <h3 class="cate_child_title">Sản phẩm tìm kiếm <span>(<?php {{ echo $numTotalProd; }} ?> sản phẩm)</span></h3>
                            </div>
                        ...
                        <section class="product_box container_bg_sc">
                            <div class="option_box_wrap grid_row justify_content_between align_items_center">
                                <div class="sort_box_holder d_flex align_items_center">
                                    ...
                                    <ul class="list_sort d_flex">
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$searchUrl}{$brandUrl}{$priceUrl}/sort=lastest{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "lastest" ? "active" : ''; }} ?>">Hàng mới</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$searchUrl}{$brandUrl}{$priceUrl}/sort=bestsellers{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "bestsellers" ? "active" : ''; }} ?>">Bán chạy nhất</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$searchUrl}{$brandUrl}{$priceUrl}/sort=priceasc{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "priceasc" ? "active" : ''; }} ?>">Giá tăng dần</a>
                                        </li>
                                        <li class="sort_item">
                                            <a href="<?php {{ echo Config::getBaseUrlClient("{$searchUrl}{$brandUrl}{$priceUrl}/sort=pricedesc{$pageUrl}"); }} ?>" class="sort_view_link <?php {{ echo $sort_vl == "pricedesc" ? "active" : ''; }} ?>">Giá giảm dần</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="wrap-list-product-catalog grid_row">
                                <?php foreach($listProdBySearch as $prodItem) : ?>
                            ...
                            {{
                                if($totalPage > 1) { echo Pagination::getPagination("{$searchUrl}{$brandUrl}{$priceUrl}{$sortUrl}/page=", $totalPage, $page); }
                            }} ?></div>
                        </section>
                        <?php else: ?>
                            <div class="cart_empty" style="margin: 50px auto; text-align: center;">
                                <img width="150" style="margin: 0 auto;" src="<?php {{ echo Config::getBaseUrlClient("public/images/icon/trang-khong-ton-tai.png"); }} ?>" alt="Không tìm thấy sản phẩm nào">
                                <p style="font-size: .9rem;">Không tìm thấy sản phẩm nào !</p>
