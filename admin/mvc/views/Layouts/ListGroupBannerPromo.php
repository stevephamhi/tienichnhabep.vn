<main class="main_content">
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                <h1>Danh sách banner khuyến mãi</h1>
                <ol class="breadcrumb d_flex align_items_center">
                    <li>
                        <a href="Home">Trang chủ</a>
                    </li>
                    <li class="active">
                        <a href="javascript:;">Danh sách banner khuyến mãi</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="GroupBannerPromo/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "GroupBannerPromo/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "GroupBannerPromo/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "GroupBannerPromo/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "GroupBannerPromo/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                <div class="page_action_item search grid_column_4">
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100" data-type="Promo">
                        ...
                        <thead>
                            <tr>
                                ...
                                <td class="text_left" style="padding: 0 10px;">
                                    ...
                                    {{ echo "GroupBannerPromo/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Nhóm group banner</span>
                                    </a>
                                </td>
                                <td>Start Date</td>
                                <td>End Date</td>
                            ...
                            <?php {{
                                if(!empty($listGroupBanner)) {
                                    ...
                                    foreach($listGroupBanner as $bannerItem) {
                                        $bannerItem['url_update'] = "GroupBannerPromo/update/{$bannerItem['bannerGroup_id']}/".Format::create_slug($bannerItem['bannerGroup_name']).".html";
                                    ?>
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("GroupBannerBottom/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
