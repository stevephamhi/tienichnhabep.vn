    ...
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            ...
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="Brand/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "Brand/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "Brand/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "Brand/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "Brand/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
    ...
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                ...
                <div>Tổng số <strong>(<?php {{ echo $totalBrand; }} ?>)</strong> Thương hiệu</div>
            </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{ echo "Brand/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Tên thương hiệu</span>
                                    </a>
                                </td>
                                <td>Thứ tự</td>
                                <td>Trạng thái</td>
                        ...
                        <tbody>
                            <?php if (!empty($listBrand)) : ?>
                                <?php $orderRow = 1; foreach ( $listBrand as $brandItem ) : ?>
                                    <?php {{ $brandItem['url_update'] = "Brand/update/{$brandItem['brand_id']}/".Format::create_slug($brandItem['brand_name']).".html"; }} ?>
                                    <tr data-id="<?php {{ echo $brandItem['brand_id']; }} ?>">
                                        <td>
                                            <input class="checkItem" type="checkbox" name="<?php {{ echo $brandItem['brand_id']; }} ?>">
                                        </td>
                                        <td><?php {{ echo $orderRow++; }} ?></td>
                                        <td class="image">
                                            <img src="<?php {{ echo $brandItem['brand_image']; }} ?>" alt="">
                                        </td>
                                        <td><?php {{ echo $brandItem['brand_name']; }} ?></td>
                                        <td><?php {{ echo $brandItem['brand_order']; }} ?></td>
                                        <td>
                                            <div class="toogle_status <?php {{ echo $brandItem['brand_status']; }} ?> position_relative">
                                        ...
                                        <td class="update">
                                            <a href="<?php {{ echo $brandItem['url_update']; }} ?>">
...
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/app/brandAjax.js"); }} ?>"></script>
