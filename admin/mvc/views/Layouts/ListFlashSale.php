<main class="main_content">
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                <h1>Danh mục flash sale</h1>
                <ol class="breadcrumb d_flex align_items_center">
                    <li>
                        <a href="?">Trang chủ</a>
                    </li>
                    <li class="active">
                        <a href="javascript:;">Danh sách flash sale</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="FlashSale/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "FlashSale/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "FlashSale/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "FlashSale/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "FlashSale/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                    ...
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100">
                            <div class="form_group position_relative">
                                <input type="text" name="searchStr" disabled class="form_control" value="<?php {{ echo $strSearch; }} ?>" placeholder="Nhập tên sản phẩm ..." autocomplete="off" spellcheck="false">
                                <button type="submit" name="" disabled class="form_button position_absolute">
                                    <i class="fa fa-search" aria-hidden="true"></i>
                                    <span>Tìm kiếm</span>
                                </button>
                            </div>
    ...
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                ...
                <div>Tổng số <strong>(<?php {{ echo $totalFlashSale; }} ?>)</strong> danh mục</div>
            </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{ echo "FlashSale/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Ngày bắt đầu</span>
                                    </a>
                                </td>
                                <td>Ngày kết thúc</td>
                        ...
                        <tbody>
                            <?php if(!empty($listFlashSale)) : ?>
                                <?php $orderRow = 1; foreach( $listFlashSale as $flashSaleItem ) : ?>
                                    <?php {{ $flashSaleItem['url_update']    = "FlashSale/update/{$flashSaleItem['prod_flashsale_id']}"; }} ?>
                                    <?php {{ $flashSaleItem['url_goto_prod'] = "Product/update/{$flashSaleItem['prod_flashsale_prodId']}/". Format::create_slug($flashSaleItem['prod_flashsale_prodName']).".html"; }} ?>
                                    <tr data-id="<?php {{ echo $flashSaleItem['prod_flashsale_id']; }} ?>">
                                        <td>
                                            <input class="checkItem" type="checkbox" name="<?php {{ echo $flashSaleItem['prod_flashsale_id']; }} ?>">
                                        </td>
                                        <td><?php {{ echo $orderRow ++; }} ?></td>
                                        <td><?php {{ echo $flashSaleItem['prod_flashsale_customerGroupName']; }} ?></td>
                                        <td class="image">
                                            <img src="<?php {{ echo $flashSaleItem['prod_flashsale_image']; }} ?>" alt="<?php {{ echo $flashSaleItem['prod_flashsale_prodName']; }} ?>">
                                        </td>
                                        <td>
                                            <a class="name" href="<?php {{ echo $flashSaleItem['url_goto_prod']; }} ?>">
                                                <?php {{ echo $flashSaleItem['prod_flashsale_prodName']; }} ?>
                                            </a>
                                        </td>
                                        <td><?php {{ echo Format::formatTime($flashSaleItem['prod_flashsale_dateStart']); }} ?></td>
                                        <td><?php {{ echo Format::formatTime($flashSaleItem['prod_flashsale_dateEnd']); }} ?></td>
                                        <td><?php {{ echo Format::formatCurrency($flashSaleItem['prod_flashsale_price']); }} ?></td>
                                        <td><?php {{ echo Format::formatCurrency($flashSaleItem['prod_flashsale_currentPrice']); }} ?></td>
                                        <td><?php {{ echo $flashSaleItem['prod_flashsale_order']; }} ?></td>
                                        <td>
                                            <div class="toogle_status <?php {{ echo $flashSaleItem['prod_flashsale_status']; }} ?> position_relative">
                                        ...
                                        <td class="update">
                                            <a href="<?php {{ echo $flashSaleItem['url_update']; }} ?>">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="15">Hiện tại không có sản phẩm nào trong chương trình flash sale</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("FlashSale/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/config/jquery.min.js"); }} ?>"></script>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/app/flashSale.ajax.js"); }} ?>"></script>
