<main class="main_content">
        ...
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                ...
                <ol class="breadcrumb d_flex align_items_center">
                    ...
                    <li class="active">
                        <a href="javascript:;">Danh sách danh mục tin tức</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="CateNews/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "CateNews/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "CateNews/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "CateNews/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "CateNews/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                    ...
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100">
                            <div class="form_group position_relative">
                                <input type="text" name="searchStr" class="form_control" value="<?php {{ echo $strSearch; }} ?>" placeholder="Nhập tên danh mục tin tức ..." autocomplete="off" spellcheck="false">
    ...
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                ...
                <div>Tổng số <strong>(<?php {{ echo $totalCateNews; }} ?>)</strong> danh mục</div>
            </div>
            <div class="panel_body">
                <div id="table_content">
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <td class="text_left" style="padding: 0 10px;">
                                    ...
                                    {{ echo "CateNews/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Tên danh mục</span>
                                    </a>
                                </td>
                                <td>Trạng thái</td>
                                <td>Cập nhật</td>
                        ...
                        <tbody>
                        <?php {{ if(!empty($listCateNews)) {
                                $orderRow = 1;
                                foreach($listCateNews as $cateNewsItem) {
                                    $cateNewsItem['url_update'] = "CateNews/update/{$cateNewsItem['cateNews_id']}/".($cateNewsItem['cateNews_seoUrl']).".html";
                                ?>
                                    <tr data-id="<?php {{ echo $cateNewsItem['cateNews_id']; }} ?>">
                                        <td>
                                            <input class="checkItem" type="checkbox" name="<?php {{ echo $cateNewsItem['cateNews_id']; }} ?>">
                                        </td>
                                        <td><?php {{ echo $orderRow++; }} ?></td>
                                        <td class="text_left"><?php {{ echo $cateNewsItem['cateNews_name']; }} ?></td>
                                        <td>
                                            <div class="toogle_status <?php {{ echo $cateNewsItem['cateNews_status']; }} ?> position_relative">
                                        ...
                                        <td class="update">
                                            <a href="<?php {{ echo $cateNewsItem['url_update']; }} ?>">
                                ...
                                ?>
                                <tr>
                                    <td colspan="7">Chưa có danh tin tức nào !</td>
                                </tr>
                            <?php } }} ?>
                        </tbody>
                    </table>
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("CateNews/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/config/jquery.min.js"); }} ?>"></script>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/app/cateNewsAjax.js"); }} ?>"></script>
