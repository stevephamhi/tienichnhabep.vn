<main class="main_content">
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            ...
            <div class="d_flex align_items_center">
                ...
                <a class="btn_item btn_primary" href="News/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "News/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "News/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "News/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "News/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
    ...
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                ...
                <div>Tổng số <strong>(<?php {{ echo $totalNews; }} ?>)</strong> Tin tức</div>
            </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <td class="text_left" style="padding: 0 10px;">
                                    ...
                                    {{ echo "News/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Tên tin tức</span>
                                    </a>
                                </td>
                                <td>Trạng thái</td>
                                <td>Cập nhật</td>
                        ...
                        <tbody>
                            ...
                            foreach($listNews as $newsItem) {
                                $newsItem['url_update'] = "News/update/{$newsItem['news_id']}/".($newsItem['news_seoUrl']).".html";
                            ?>
                                <tr data-id="<?php {{ echo $newsItem['news_id']; }} ?>">
                                    <td>
                                        <input class="checkItem" type="checkbox" name="<?php {{ echo $newsItem['news_id']; }} ?>">
                                    </td>
                                    <td><?php {{ echo $orderRow++; }} ?></td>
                                    <td class="image">
                                        <img src="<?php {{ echo $newsItem['news_image']; }} ?>" alt="">
                                    </td>
                                    <td class="info" style="width: 500px;">
                                        <a href="javascript:;" class="name"><?php {{ echo $newsItem['news_name']; }} ?></a>
                                        <p style="margin-top: 5px;">Xem: <?php {{ echo $newsItem['news_views']; }} ?>, like: <?php {{ echo $newsItem['news_likes']; }} ?></p>
                                    </td>
                                    <td>
                                        <div class="toogle_status <?php {{ echo $newsItem['news_status']; }} ?> position_relative">
                                    ...
                                    <td class="update">
                                        <a href="<?php {{ echo $newsItem['url_update']; }} ?>">
                            ...
                            <?php
                            } } else { ?>  <td colspan="9">Chưa có bản tin nào !</td> <?php } }} ?>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("News/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
