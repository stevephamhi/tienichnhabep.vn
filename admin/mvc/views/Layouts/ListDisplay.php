<main class="main_content">
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                <h1>Bố cục trang chủ</h1>
                <ol class="breadcrumb d_flex align_items_center">
                    <li>
                        <a href="">Trang chủ</a>
                    </li>
                    <li class="active">
                        <a href="javascript:;">Danh sách bố cục</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="Display/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_success" href="VideoGroup">
                    <i class="fa fa-video-camera" aria-hidden="true"></i>
                    <span>Video</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "Display/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "Display/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "Display/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "Display/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                    ...
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100">
                            <div class="form_group position_relative">
                                <input type="text" name="searchStr" class="form_control" value="<?php {{ echo $strSearch; }} ?>" placeholder="Nhập tên bản tin ..." autocomplete="off" spellcheck="false">
    ...
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                ...
                <div>Tổng số <strong>(<?php {{ echo $totalDisplay; }} ?>)</strong> bố cục</div>
            </div>
                        ...
                        <thead>
                            <tr>
                                ...
                                <td class="text_left" style="padding: 0 10px;">
                                    ...
                                    {{ echo "Display/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Số thứ tự</span>
                                    </a>
                                </td>
                                <td>Trạng thái</td>
                                <td>Cập nhật</td>
                        ...
                        <tbody>
                            <?php {{
                                if(!empty($listDisplay)) {
                                    $orderRow = 1;
                                    foreach($listDisplay as $displayItem) {
                                        $displayItem['url_update'] = "Display/update/{$displayItem['display_id']}/".Format::create_slug($displayItem['display_title']).".html";
                                        ?>
                                        <tr data-id="<?php {{ echo $displayItem['display_id']; }} ?>">
                                            <td>
                                                <input class="checkItem" type="checkbox" name="<?php {{ echo $displayItem['display_id']; }} ?>">
                                            </td>
                                            <td><?php {{ echo $orderRow ++; }} ?></td>
                                            <td class="text_left" style="width: 300px;"><?php {{ echo $displayItem['display_title']; }} ?></td>
                                            <td><?php {{ echo $displayItem['display_type']; }} ?></td>
                                            <td>
                                                <input class="form_control" style="width: 40px; text-align: center;" type="text" value="<?php {{ echo $displayItem['display_order']; }} ?>" autocomplete="off" spellcheck="false">
                                                <button type="button" class="form_button save_display_order" data-order="<?php {{ echo $displayItem['display_order']; }} ?>">
                                                    <i class="fa fa-refresh" aria-hidden="true"></i>
                                                </button>
                                            </td>
                                            <td>
                                                <div class="toogle_status <?php {{ echo $displayItem['display_status']; }} ?> position_relative">
                                            ...
                                            <td class="update">
                                                <a href="<?php {{ echo $displayItem['url_update']; }} ?>">
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("Display/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/config/jquery.min.js"); }} ?>"></script>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/app/listDisplay.ajax.js"); }} ?>"></script>
