<main class="main_content">
        ...
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                ...
                <ol class="breadcrumb d_flex align_items_center">
                    ...
                    <li class="active">
                        <a href="javascript:;">Danh sách danh mục</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                ...
                <a class="btn_item btn_success" href="CateProduct/analysis">
                    <i class="fa fa-line-chart" aria-hidden="true"></i>
                    <span>Phân tích</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "CateProduct/index/{$orderBy}/{$status}/{$page}"; }} ?>">
            ...
            <div class="action_wrap d_flex align_items_center">
                ...
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/index/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/index/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/index/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                <div class="page_action_item search grid_column_4">
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100" data-action="index">
            ...
            <div class="panel_body">
                <div id="table_content">
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <td>
                                    ...
                                    {{ echo "CateProduct/index/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Tên danh mục</span>
                                    </a>
                                </td>
                                <td>Trạng thái</td>
                                <td>Cập nhật</td>
                        ...
                        <tbody>
                            <?php {{ if(!empty($listCateProducts)) {
                                $orderRow = 1;
                                foreach($listCateProducts as $cateProdItem) {
                                    $cateProdItem['url_update'] = "CateProduct/update/{$cateProdItem['cateProd_id']}/".($cateProdItem['cateProd_seoUrl']).".html";
                                ?>
                                    <tr data-id="<?php {{ echo $cateProdItem['cateProd_id']; }} ?>">
                                        <td>
                                            <input class="checkItem" type="checkbox" name="<?php {{ echo $cateProdItem['cateProd_id']; }} ?>">
                                        </td>
                                        ...
                                        <td class="update">
                                            <a href="<?php {{ echo $cateProdItem['url_update']; }} ?>">
                                ...
                                ?>
                                <tr>
                                    <td colspan="7">Chưa có danh mục sản phẩm nào !</td>
                                </tr>
                            <?php } }} ?>
                        </tbody>
                    </table>
    ...
    {{
        if($totalPage > 1) { echo Pagination::getPagination("CateProduct/index/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
