<main class="main_content">
    <div class="page_header">
        <div class="container_fluid d_flex justify_content_between align_items_center">
            <div class="d_flex align_items_end">
                <h1>Danh mục sản phẩm</h1>
                <ol class="breadcrumb d_flex align_items_center">
                    <li>
                        <a href="?">Trang chủ</a>
                    </li>
                    <li class="active">
                        <a href="javascript:;">Phân tích</a>
                    </li>
                </ol>
            </div>
            <div class="d_flex align_items_center">
                <a class="btn_item btn_primary" href="CateProduct/add">
                    <i class="fa fa-plus" aria-hidden="true"></i>
                    <span>Thêm mới</span>
                </a>
                <a class="btn_item btn_success" href="CateProduct">
                    <i class="fa fa-list" aria-hidden="true"></i>
                    <span>Danh sách</span>
                </a>
                <a class="btn_item btn_default" href="<?php {{ echo "CateProduct/analysis/{$orderBy}/{$status}/{$page}"; }} ?>">
                    <i class="fa fa-refresh" aria-hidden="true"></i>
                    <span>Làm mới</span>
                </a>
            </div>
        </div>
        <div class="container_fluid">
            <div class="action_wrap d_flex align_items_center">
                <div class="page_action_item filter grid_column_4">
                    <div class="value d_flex align_items_center justify_content_center">
                        <a class="item <?php {{ echo $status == 'all' ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/analysis/{$orderBy}/all/{$page}"; }} ?>">Tất cả</a>
                        <a class="item <?php {{ echo $status == 'on'  ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/analysis/{$orderBy}/on/{$page}"; }} ?>">Bật</a>
                        <a class="item <?php {{ echo $status == 'off' ? 'active' : null; }} ?>" href="<?php {{ echo "CateProduct/analysis/{$orderBy}/off/{$page}"; }} ?>">Tắt</a>
                    </div>
                </div>
                <div class="page_action_item search grid_column_4">
                    <div class="value d_flex align_items_center w_100 position_relative">
                        <form action="" class="search_module w_100" data-action="analysis">
                            <div class="form_group position_relative">
                                <input type="text" name="searchStr" class="form_control" value="<?php {{ echo $strSearch; }} ?>" placeholder="Nhập tên danh mục sản phẩm ..." autocomplete="off" spellcheck="false">
                                <button type="submit" name="" class="form_button position_absolute">
                                    <i class="fa fa-search" aria-hidden="true"></i>
                                    <span>Tìm kiếm</span>
                                </button>
                            </div>
                        </form>
                        <div class="rearch_recomment position_absolute">
                            <span class="close position_absolute">
                                <i class="fa fa-times" aria-hidden="true"></i>
                            </span>
                            <ul class="search_recomment_wrap_list"></ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="loader_wrap">
        <div class="loader"></div>
    </div>
    <div class="alert_wrap">
        <div class="alert  position_relative" data-status="">
            <i class="fa fa-check-circle" style="margin-right: 5px;"></i>
            <span></span>
            <button type="button" class="close position_absolute">x</button>
        </div>
    </div>
    <div class="table_content container_fluid">
        <div class="panel_table">
            <div class="panel_heading d_flex justify_content_between align-center">
                <h2 class="panel_title">
                    <i class="fa fa-list"></i>
                    <span>Phân tích</span>
                </h2>
                <div>Tổng số <strong>(<?php {{ echo $totalCateProd; }} ?>)</strong> danh mục</div>
            </div>
            <div class="panel_body">
                <div id="table_content">
                    <table class="table">
                        <thead>
                            <tr>
                                <td>STT</td>
                                <td>Hình ảnh</td>
                                <td>Tên danh mục</td>
                                <td>
                                    <a class="<?php {{ echo $orderBy; }} ?>" href="<?php
                                    {{ $__orderBy__ = $orderBy == "desc" ? "asc" : "desc"; }}
                                    {{ echo "CateProduct/analysis/{$__orderBy__}/{$status}/{$page}"; }} ?>"><span>Lượt truy cập</span>
                                    </a>
                                </td>
                                <td>Tổng sản phẩm</td>
                                <td>Trạng thái</td>
                                <td>Doanh mục nổi bậc</td>
                            </tr>
                        </thead>
                        <tbody>
                            <?php {{
                                if(!empty($listCateProducts)) {
                                    $orderRow = 1;
                                    foreach($listCateProducts as $cateProdItem) { ?>
                                        <tr data-id="<?php {{ echo $cateProdItem['cateProd_id']; }} ?>">
                                            <td><?php {{ echo $orderRow ++; }} ?></td>
                                            <td class="image">
                                                <img src="<?php {{ echo $cateProdItem['cateProd_image']; }} ?>" alt="<?php {{ echo $cateProdItem['cateProd_name']; }} ?>">
                                            </td>
                                            <td><?php {{ echo $cateProdItem['cateProd_name']; }} ?></td>
                                            <td>
                                                <input class="form_control" min="0" style="width: 100px;" type="number" value="<?php {{ echo $cateProdItem['cateProd_views']; }} ?>" data-value="<?php {{ echo $cateProdItem['cateProd_views']; }} ?>">
                                                <button type="button" class="form_button saveNumViewCateProd"><i class="fa fa-refresh" aria-hidden="true"></i></button>
                                            </td>
                                            <td><?php {{ echo $cateProdItem['cateProd_numTotalProd_ties']; }} ?></td>
                                            <td>
                                                <div class="toogle_status <?php {{ echo $cateProdItem['cateProd_status']; }} ?> position_relative">
                                                    <div class="toggle_group position_absolute">
                                                        <label class="toogle_on btn">Bật</label>
                                                        <label class="toogle_off btn">Tắt</label>
                                                        <span class="toggle_handle"></span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <label for="cateProd_hot_<?php {{ echo $cateProdItem['cateProd_id']; }} ?>" class="form_button">
                                                    <input type="checkbox" <?php {{ echo $cateProdItem['cateProd_hot'] == "1" ? "checked" : null; }} ?> class="select_cateProd_hot" id="cateProd_hot_<?php {{ echo $cateProdItem['cateProd_id']; }} ?>">
                                                    <span class="btn_success" style="padding: 1px 3px; border-radius: 5px;">Chọn</span>
                                                </label>
                                            </td>
                                        </tr>
                                    <?php }
                                }
                            }} ?>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    <div class="pagination_wrap"><?php
    {{
        if($totalPage > 1) { echo Pagination::getPagination("CateProduct/analysis/{$orderBy}/{$status}/", $totalPage, $page); }
    }} ?></div>
</main>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/config/jquery.min.js"); }} ?>"></script>
<script type="text/javascript" src="<?php {{ echo $base->getBaseURLAdmin("public/js/app/cateProductAjax.js"); }} ?>"></script>
<script type="text/javascript" class="handle_recomment_search">
    // ========== ########## HANDLE RECOMMENT SEARCH ########## ========== //
    let inputFocusSearchEL  = document.querySelector(".action_wrap .page_action_item.search input[name='searchStr']");
    let recommentSearchEl   = document.querySelector(".rearch_recomment");
    let btnCloseRecommentEl = document.querySelector(".rearch_recomment .close");
    let listRecommentItemEL = document.querySelectorAll(".rearch_recomment .search_recomment_wrap_list li span");
    inputFocusSearchEL.addEventListener('focus', function() {
        recommentSearchEl.style.display = "block";
    });
    btnCloseRecommentEl.addEventListener('click', function() {
        recommentSearchEl.style.display = "none";
    });
    listRecommentItemEL.forEach(el => {
        el.addEventListener('click', function() {
            inputFocusSearchEL.value = el.textContent;
        });
    });
</script>
<script type="text/javascript" class="handle_toggle_input_status">
    let btnToggle = document.querySelectorAll("#table_content table.table tr .toogle_status");
    btnToggle.forEach(el => {
        el.addEventListener('click', function() {
            if(this.classList.contains('on')) {
                this.classList.remove('on');
                this.classList.add('off');
            } else {
                this.classList.remove('off');
                this.classList.add('on');
            }
        });
    });
</script>